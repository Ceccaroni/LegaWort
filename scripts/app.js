...
function manifestItemToEntry(item) {
  ...
  if (typeof item === 'string') {
    return { wort: item, wortart: null, def: null, tags: [] };
  }

  if (typeof item === 'object' && typeof item.wort === 'string') {
    return {
      wort: item.wort,
      wortart: item.wortart || null,
      def: item.def || null,
      tags: Array.isArray(item.tags) ? item.tags : []
    };
  }

  if (typeof item === 'object' && typeof item.lemma === 'string') {
    return {
      wort: item.lemma,
      wortart: item.pos || null,
      def: item.def || null,
      tags: Array.isArray(item.tags) ? item.tags : []
    };
  }
...
function manifestEntryToCard(manifestEntry){
  if(!manifestEntry || typeof manifestEntry.wort !== 'string') return null;
  const wort = manifestEntry.wort.trim();
  if(!wort) return null;
  const baseTags = Array.isArray(manifestEntry.tags)
    ? manifestEntry.tags.map(tag => String(tag).trim()).filter(Boolean)
    : [];
  const card = {
    wort,
    erklaerung: typeof manifestEntry.def === 'string' ? manifestEntry.def.trim() : '',
    beispiele: [],
    tags: baseTags.slice()
  };
  if(manifestEntry.wortart){
    card.def_src = { pos: manifestEntry.wortart };
    if(!card.tags.length){
      card.tags.push(manifestEntry.wortart);
    }
  }
  card._matchKey = matchKey(card.wort);
  return card;
}
  ...
  try{
    ...
    outer: for(const list of lists){
      ...
      for(const item of list){
        const normalized = manifestItemToEntry(item);
        if(!normalized || !normalized.wort) continue;
        const entry = manifestEntryToCard(normalized);
        if(!entry) continue;
        const key = entryKey(entry);
        if(!key) continue;
        if(dedup.has(key)){
          const existing = dedup.get(key);
          if(entry.erklaerung && !existing.erklaerung){
            existing.erklaerung = entry.erklaerung;
          }
          if(entry.def_src && (!existing.def_src || !existing.def_src.pos)){
            existing.def_src = Object.assign({}, existing.def_src, entry.def_src);
          }
        ...
        dedup.set(key, entry);
        if(dedup.size >= SEARCH_POOL_LIMIT){
          break outer;
        }
      }
    }
...
async function runSearch(input){
  ...
  if(window.LEGA_FLAGS?.V2_MANIFEST_SEARCH){
    ...
    const manifestRankFn = (entriesFromManifest, queryKey)=>{
      const entries = [];
      if(!Array.isArray(entriesFromManifest) || !entriesFromManifest.length){
        return entries;
      }
      for(const baseEntry of entriesFromManifest){
        if(!baseEntry || typeof baseEntry !== 'object') continue;
        if(typeof baseEntry.wort !== 'string') continue;
        const existingKey = baseEntry._matchKey || matchKey(baseEntry.wort);
        if(!existingKey) continue;
        baseEntry._matchKey = existingKey;
