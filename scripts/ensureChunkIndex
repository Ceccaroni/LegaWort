...
function collectBuckets(queryNorm){
  ...
  const basePrefix = q.slice(0, 2);  // neuer fixer Präfix: 2 Zeichen
  return {
    basePrefix,
    fallbackPrefixes: []
  };
...
function collectBucketsV2(queryNorm, options){
  ...
  // Fester Präfix für neue Manifest-Struktur: 2 Zeichen
  const prefixLen = 2;

  // Mindestlänge für Präfixe
  const minLength = 2;

  // Limit für fallback-Prefixes
  ...
  // Lautvarianten für häufige Verwechslungen
