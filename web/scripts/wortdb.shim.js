...
function matchPrefix(w){
  const normalized = norm(w).replace(/[^a-z0-9]/g, "_");
...
async function loadDefinition(w){
  const prefix = matchPrefix(w);
  const file = `${BASE}/${prefix}/${safeBase(w)}.json`;
  try {
...
async function fetchLocalDef(w){
  const data = await loadDefinition(w);
  if (!data) throw new Error(`404 ${defPath(w)}`);
  normalizeSilbenField(data);
  await applyHyphenation(w, data);
  console.info("def local fetch", w, defPath(w));
  return data;
}
